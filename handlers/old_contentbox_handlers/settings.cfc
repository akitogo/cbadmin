...
component  {
	...
	// Dependencies
	property name="settingsService" inject="settingService@cbadmin";
	property name="LoginTrackerService" inject="LoginTrackerService@cbadmin";
	...
	function preFlightCheck( event, currentAction, rc, prc ){
		settingsService.preFlightCheck();
		writedump('done');
		abort;
	}
	/**
	...
	function rawtable( event, rc, prc ){
		...
		prc.oPaging    = getInstance( "Paging@cbadmin" );
	...
	function authLogs( event, rc, prc ){
		prc.featureEnabled = prc.cbsettings.cbadmin_security_login_blocker;

...
<!------------------------------------------- CONSTRUCTOR ------------------------------------------->	
    ...
    <cffunction name="init" access="public" returntype="Paging" output="false">
		<cfargument name="settingService" required="true" inject="id:settingService@cbadmin"/>
		<cfscript>

  		// Setup Paging Properties
  		setPagingMaxRows( arguments.settingService.getSetting( "cbadmin_paging_maxrows",10 ) );
  		setPagingBandGap( arguments.settingService.getSetting( "cbadmin_paging_bandgap",1 ) );

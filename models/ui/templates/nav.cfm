...
<cfloop array="#local.thisMenu#" index="local.topMenu">
	<cfif !len( local.topMenu.permissions ) OR prc.oUser.checkPermission( local.topMenu.permissions )>
	<li class="nav-dropdown<cfif event.getValue( name='tab#local.topMenu.name#', defaultValue=false, private=true )> active open</cfif>#local.topMenu.class#" data-name="#local.topMenu.name#">
		
		<a 	href="#( isCustomFunction( local.topMenu.href ) ? local.topMenu.href() : local.topMenu.href )#" 
			...
			<cfif structKeyExists( local.topMenu, "data" ) && structCount( local.topMenu.data )>#parseADataAttributes( local.topMenu.data )#</cfif>>#( isCustomFunction( local.topMenu.label ) ? local.topMenu.label() : local.topMenu.label )# <cfif arrayLen( local.topMenu.subMenu )></cfif></a>
		...
		<cfloop array="#local.topMenu.submenu#" index="local.thisSubMenu">
			<!--- Security --->
			<cfif !len(local.thisSubMenu.permissions) OR prc.oUser.checkPermission( local.thisSubMenu.permissions )>

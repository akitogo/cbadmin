...
component singleton{
	...
	*/
	function renderAvatar( required email, numeric size=80, string class="gravatar" ){
		...
		// check settings
		if( !structKeyExists(prc.cbSettings,'cbadmin_gravatar_display') || !prc.cbSettings.cbadmin_gravatar_display ){ return ""; }

		// render it out
		savecontent variable="avatar"{
			writeOutput('<img class="#arguments.class#" align="middle" width="#arguments.size#" height="#arguments.size#" src="//www.gravatar.com/avatar.php?gravatar_id=#lcase(Hash(emailTarget))#&s=#arguments.size#&r=#prc.cbSettings.cbadmin_gravatar_rating#" />');
		}

...
component extends="coldbox.system.Interceptor"{
	...
	property name="securityService"		inject="id:securityService@cbadmin";
	property name="loginTrackerService"	inject="id:loginTrackerService@cbadmin";
	property name="systemUtil"			inject="SystemUtil@cbadmin";
	...
	*/
	function cbadmin_preLogin( event, data, buffer ){
		...
		if( !settingService.getSetting( "cbadmin_security_login_blocker" ) ){ return; }
	...
	*/
	function cbadmin_onLogin( event, data, buffer ){
		...
		var prc = event.getCollection( private = true );
		// get logged in user
		var oUser = securityService.getUserSession();
		// Build entry to log

...
component extends="cborm.models.VirtualEntityService" singleton{
	...
	*/
	boolean function deletePermission( required permissionId ){
		/* This is no longer needed as we've defined the relationships between the models.
		transaction{
			// We do SQL deletions as those relationships are not bi-directional
			// delete role relationships
			var q = new Query(sql="delete from cbadmin_rolePermissions where FK_permissionId = :permissionId" );
			q.addParam(name="permissionId", value=arguments.permissionId, cfsqltype="numeric" );
			q.execute();

			// delete group relationships
			var q = new Query(sql="delete from cbadmin_groupPermissions where FK_permissionId = :permissionId" );
			q.addParam(name="permissionId", value=arguments.permissionId, cfsqltype="numeric" );
			q.execute();

			// delete user relationships
			var q = new Query(sql="delete from cbadmin_userPermissions where FK_permissionId = :permissionId" );
			q.addParam(name="permissionId", value=arguments.permissionId, cfsqltype="numeric" );
			q.execute();

			// delete permission now
			var q = new Query(sql="delete from cbadmin_permission where permissionId = :permissionId" );
			q.addParam(name="permissionId", value=arguments.permissionId, cfsqltype="numeric" );
			q.execute();
		}
		*/

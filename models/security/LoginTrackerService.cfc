...
component extends="cborm.models.VirtualEntityService" singleton{
	...
	*/
	boolean function isblocked( LoginAttempt attempt ){
		var max_attempts 	= settingService.getSetting( 'cbadmin_security_max_attempts' );
		var max_blockTime 	= settingService.getSetting( 'cbadmin_security_blocktime' );
	...
	*/
	LoginTrackerService function truncate(){
		var q = new Query( sql="truncate cbadmin_loginAttempts" ).execute();
		return this;
	}
	...
	*/
	LoginTrackerService function rotate(){
		var maxLogs 	= settingService.getSetting( "cbadmin_security_max_auth_logs" );
		var maxLogs 	= 4;
		var totalLogs  	= count();
	...
	*/
	LoginTrackerService function reset(){
		var limit = dateAdd( 'n', settingService.getSetting( 'cbadmin_security_blocktime' ) * -1, now() );

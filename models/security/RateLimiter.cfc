...
component extends="coldbox.system.Interceptor" {
	...
	// DI
	property name="settingService" inject="id:settingService@cbadmin";
	...
	function onRequestCapture( event, data, buffer ){
		...
		if (
			!structKeyExists( allSettings, "cbadmin_security_rate_limiter" ) || allSettings.cbadmin_security_rate_limiter == false
		) {
			return false;
		}

		// do we limit bot OR normal requests as well?
		if ( !len( cgi.http_cookie ) OR !allSettings.cbadmin_security_rate_limiter_bots_only ) {
			// limit it now.
			limiter(
				count    = allSettings[ "cbadmin_security_rate_limiter_count" ],
				duration = allSettings[ "cbadmin_security_rate_limiter_duration" ],
	...
	private function limiter( count, duration, event, settings ){
		...
		if ( dateDiff( "s", targetData.lastAttempt, now() ) LT arguments.duration ) {
			...
			if ( targetData.attempts GT arguments.count ) {
				if ( settings.cbadmin_security_rate_limiter_logging && log.canInfo() ) {
					// Log it to app logs
					log.info(
				...
				if ( len( settings.cbadmin_security_rate_limiter_redirectURL ) ) {
					location(
						settings.cbadmin_security_rate_limiter_redirectURL,
						"false",
						"301"
					);
					return;
				...
				writeOutput(
					replaceNoCase(
						settings[ "cbadmin_security_rate_limiter_message" ],
						"{duration}",
						arguments.duration,
						"all"
					)
